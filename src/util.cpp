...
const char * const BITCOIN_CONF_FILENAME = "qbic.conf";
const char * const BITCOIN_PID_FILENAME = "qbicd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "qbic" is a composite category enabling all Qbic-related debug output
            if(ptrCategory->count(string("qbic"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\QbicCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\QbicCore
    // Mac: ~/Library/Application Support/QbicCore
    // Unix: ~/.qbiccore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "QbicCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/QbicCore";
#else
    // Unix
    return pathRet / ".qbiccore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty qbic.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override qbic.conf
